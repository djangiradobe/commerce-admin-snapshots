...
async function main(params) {
    ...
    try {
        ...
        const snap = await (async () => {
            try {
                return await snapCol.findOne({ _id: snapshotId });
            }
            catch {
                return null;
            }
        ...
        if (!snap)
            return errorResponse(404, `Snapshot ${snapshotId} not found`, logger);
        ...
        const currentSchemaDoc = await (async () => {
            try {
                return await schemaCol.findOne({ _id: SCHEMA_DOC_ID });
            }
            catch {
                return null;
            }
        ...
        try {
            await snapCol.insertOne(preRestoreSnap);
        }
        catch (e) {
        ...
        if (restoreSchema && snap.schema) {
            try {
                await schemaCol.updateOne({ _id: SCHEMA_DOC_ID }, { $set: { schema: snap.schema, updatedAt: now }, $setOnInsert: { _id: SCHEMA_DOC_ID, createdAt: now } }, { upsert: true });
            }
            catch (e) {
                // Fallback for drivers without upsert.
                if (currentSchemaDoc)
                    await schemaCol.updateOne({ _id: SCHEMA_DOC_ID }, { $set: { schema: snap.schema, updatedAt: now } });
                else
                    await schemaCol.insertOne({ _id: SCHEMA_DOC_ID, schema: snap.schema, createdAt: now, updatedAt: now });
            }
        }
